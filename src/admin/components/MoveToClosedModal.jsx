...
// captures the commercial terms (package, duration, closer, activation window).
import { useState, useEffect } from 'react'
import { Modal, Field, Input, Select, AdminButton, toISODate } from './shared.jsx'
import { PACKAGES, CLOSERS } from '../config.js'
...
export default function MoveToClosedModal({ open, prospect, onClose, onSubmit }) {
  ...
  const submit = async (e) => {
    e.preventDefault()
    const err = {}
    if (!values.packageBought) err.packageBought = 'Pick the package they bought'
    ...
    >
      <form id="move-form" onSubmit={submit} className="flex flex-col gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Package bought" required error={errors.packageBought}>
            <Select value={values.packageBought || ''} onChange={set('packageBought')} placeholder="Select a package">
              {PACKAGES.map((p) => (
