...
// The main inbound flow is still the brief form → Potential deals automatically.
      ...
      <form id="add-prospect-form" onSubmit={submit} className="flex flex-col gap-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Client name" required error={errors.clientName}>
            <Input value={values.clientName} onChange={set('clientName')} placeholder="e.g. NorthAir HVAC" />
          </Field>
          <Field label="Industry" required error={errors.industry}>
            <Input value={values.industry} onChange={set('industry')} placeholder="e.g. HVAC" />
          </Field>
        </div>
        <Field label="Interest in package">
          <Select value={values.interestPackage} onChange={set('interestPackage')} placeholder="Select a package">
            {PACKAGES.map((p) => (
        ...
        <Field label="Need — what to give as a prototype" hint="What we'll mock up to win them.">
          <Textarea
            ...
            value={values.need}
            onChange={set('need')}
            placeholder="e.g. Booking chatbot demo on their site"
          />
        </Field>
